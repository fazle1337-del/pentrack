...
def _sso_redirect(suffix: str) -> RedirectResponse:
    ...
    return RedirectResponse(f"{base}#{suffix}", status_code=status.HTTP_302_FOUND)
...
def sso_login():
    ...
    return RedirectResponse(
        oidc.build_authorization_url(state, nonce),
...
def sso_callback(
    ...
    if error or not code or not state:
        return _sso_redirect(f"sso_error={error or 'login_failed'}")
    try:
        nonce = oidc.read_state_token(state)
        tokens = oidc.exchange_code(code)
        claims = oidc.validate_id_token(tokens["id_token"], nonce)
        user = resolve_and_provision(db, claims)
    except NoRoleMapped:
        return _sso_redirect("sso_error=no_role")
    except LocalAccountConflict:
        # Break-glass: a local account with this email exists and is never
        # silently taken over by SSO.
        return _sso_redirect("sso_error=local_account")
    except Exception:
        # Don't leak provider/validation internals to the browser.
        return _sso_redirect("sso_error=login_failed")
    return _sso_redirect(f"sso_token={create_access_token(str(user.id))}")
